...
.rv-card:hover { box-shadow: 0 8px 28px rgba(0,0,0,0.07); transform: translateY(-2px); }

.rv-card.pending  { border-right: 4px solid #d4af37; }
.rv-card.approved { border-right: 4px solid #1a8a4a; }
.rv-card.rejected { border-right: 4px solid #ccc; opacity: 0.75; }

.rv-stars { color: #f1c40f; font-size: 1rem; letter-spacing: 2px; }
.rv-stars .empty { color: #e0d0c0; }
...
.action-pill.approve { background: #e8faf0; color: #1a8a4a; }
.action-pill.reject  { background: #fdf0f0; color: #7b1113; }
.action-pill.delete  { background: #f5f5f5; color: #888; }
.action-pill:hover   { filter: brightness(0.92); transform: scale(1.03); }
...
.stat-strip:hover { box-shadow: 0 6px 20px rgba(0,0,0,0.07); transform: translateY(-2px); }
.stat-strip.active-filter { border-color: #7b1113; background: rgba(123,17,19,0.03); }
.stat-num { font-size: 1.6rem; font-weight: 900; line-height: 1; }
.stat-lbl { font-size: 0.75rem; color: #aaa; font-weight: 700; margin-top: 2px; }
  ...
  <div class="row g-3 mb-4">
    ...
    <div class="col-6 col-md-3">
      <a href="{{ route('admin.reviews.index', $sv ? ['status'=>$sv] : []) }}"
        ...
        <div style="width:42px;height:42px;border-radius:12px;background:{{ $color }}18;color:{{ $color }};display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0">
          <i class="bi {{ $icon }}"></i>
        </div>
        <div>
      ...
      <div class="d-flex gap-3 flex-wrap align-items-center mb-4">
        ...
        <div class="me-auto" style="position:relative">
          <input type="text" name="search" value="{{ request('search') }}"
                 ...
                 class="form-control" style="width:240px;padding-left:38px">
        ...
        <div class="rv-card {{ $review->status }}">
          ...
          <div class="d-flex justify-content-between align-items-start mb-3">
            <div class="d-flex align-items-center gap-3">
              {{-- Avatar --}}
              <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#7b1113,#422018);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:800;font-size:1rem;flex-shrink:0">
                {{ mb_substr($review->reviewer_name, 0, 1) }}
              </div>
              <div>
                <div style="font-weight:800;color:#2d1e14;font-size:0.92rem">{{ $review->reviewer_name }}</div>
            ...
            {{-- Status pill --}}
            <span style="background:{{ $review->status_bg }};color:{{ $review->status_color }};font-size:0.72rem;font-weight:700;padding:4px 12px;border-radius:20px;white-space:nowrap">
              {{ $review->status_label }}
            </span>
          </div>
          ...
          <div class="rv-stars mb-2">
            ...
            @if($review->auto_approved)
              <span style="font-size:0.68rem;color:#aaa;background:#f5f5f5;padding:2px 8px;border-radius:20px">تلقائي</span>
            @endif
          </div>
          ...
          {{-- Product link --}}
          @if($review->product)
            <div style="font-size:0.77rem;color:#aaa;margin-bottom:12px;display:flex;align-items:center;gap:6px">
