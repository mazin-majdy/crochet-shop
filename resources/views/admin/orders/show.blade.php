...
@push('styles')
    <style>
        ...
        .step-active .step-circle {
            color: #fff;
            box-shadow: 0 0 0 4px rgba(255, 255, 255, 0.4), 0 0 0 6px currentColor;
        }
    ...
    <div class="container-fluid px-4">
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4 mt-2">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <span
                        style="font-size:1.3rem;font-weight:900;color:#7b1113;font-family:monospace">{{ $order->order_number }}</span>
                    <span class="ms-3"
                        ...
                        <span
                            style="width:7px;height:7px;border-radius:50%;background:currentColor;display:inline-block"></span>
                        {{ $order->status_label }}
                    </span>
                </div>
            </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8 d-flex flex-column gap-4">
                        ...
                        <div class="steps">
                            @foreach ($steps as $i => $s)
                                ...
                                <div class="step {{ $cls }}">
                                    ...
                                    <div class="step-label" style="color:{{ !$future ? $colors[$i] : '#ccc' }}">
                                        {{ $labels[$i] }}</div>
                                </div>
                                @if ($i < count($steps) - 1)
                                    <div class="step-line"
                ...
                @else
                    <div
                        style="background:#fdf0f0;border-radius:14px;padding:14px 20px;color:#7b1113;font-weight:800;display:flex;align-items:center;gap:10px">
                ...
                <div class="main-table">
                    ...
                    <div class="d-flex align-items-center gap-4 p-3 rounded-4" style="background:var(--cream)">
                        <div
                            style="width:72px;height:72px;border-radius:16px;background:#fff;display:flex;align-items:center;justify-content:center;font-size:2.5rem;border:1.5px solid #e8ddd4;flex-shrink:0;overflow:hidden">
                        ...
                        <div class="flex-grow-1">
                            <div style="font-size:1rem;font-weight:800;color:#2d1e14;margin-bottom:4px">
                        ...
                        <div class="text-end flex-shrink-0">
                            ...
                            <div style="font-size:1.05rem;font-weight:900;color:#422018">
                                {{ number_format($order->product_price) }} ₪</div>
                        </div>
                    </div>

                    {{-- Price breakdown --}}
                    <div style="margin-top:18px;padding-top:18px;border-top:1px dashed #f0e8e0">
                        @foreach ([['الكمية المطلوبة', $order->quantity . ' قطعة'], ['سعر الوحدة', number_format($order->product_price) . ' ₪']] as [$k, $v])
                            <div class="d-flex justify-content-between py-2"
                                style="border-bottom:1px solid #faf6f2;font-size:0.9rem">
                        ...
                        <div class="d-flex justify-content-between pt-3 mt-1">
                            ...
                            <span
                                style="font-size:1.4rem;font-weight:900;color:#7b1113">{{ number_format($order->total_price) }}
                                ₪</span>
                        </div>
                    </div>
                </div>
                ...
                @if ($order->notes || $order->admin_notes)
                    <div class="main-table">
                        ...
                        @if ($order->notes)
                            <div class="mb-3">
                                <div
                                    style="font-size:0.78rem;font-weight:700;color:#aaa;margin-bottom:7px;text-transform:uppercase;letter-spacing:0.5px">
                                    ملاحظات العميل</div>
                                <div
                                    style="background:var(--cream);border-radius:13px;padding:14px 18px;font-size:0.9rem;color:#444;line-height:1.8;border-right:3px solid #d4af37">
                                    {!! nl2br(e($order->notes)) !!}
                                </div>
                            </div>
                        @endif
                        @if ($order->admin_notes)
                            <div>
                                <div
                                    style="font-size:0.78rem;font-weight:700;color:#aaa;margin-bottom:7px;text-transform:uppercase;letter-spacing:0.5px">
                                    ملاحظات المدير (داخلية)</div>
                                <div
                                    style="background:#fdf8f2;border-radius:13px;padding:14px 18px;font-size:0.9rem;color:#444;line-height:1.8;border-right:3px solid #7b1113">
            ...
            <div class="col-lg-4 d-flex flex-column gap-4">
                ...
                <div class="main-table">
                    ...
                    @foreach ([['bi-person-fill', 'rgba(123,17,19,0.08)', '#7b1113', 'الاسم', $order->customer_name], ['bi-telephone-fill', 'rgba(37,211,102,0.08)', '#25D366', 'الجوال', $order->customer_phone], ['bi-envelope-fill', 'rgba(123,17,19,0.06)', '#7b1113', 'البريد', $order->customer_email], ['bi-geo-alt-fill', 'rgba(212,175,55,0.10)', '#9a7d0a', 'المدينة', $order->customer_city]] as [$icon, $bg, $color, $label, $value])
                        @if ($value)
                            <div class="info-row">
                    ...
                    <div class="d-flex flex-column gap-2 mt-4 pt-3" style="border-top:1px solid #f0e8e0">
                        @if ($order->customer_phone)
                            ...
                            <a href="https://wa.me/{{ preg_replace('/\D/', '', $order->customer_phone) }}?text={{ urlencode($message) }}"
                                target="_blank"
                ...
                <div class="main-table">
                    ...
                    <div style="display:flex;flex-direction:column;gap:12px">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <span
                                style="font-weight:800;color:#422018;font-size:0.88rem">{{ $order->payment_method_label }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center"
                            style="padding-top:10px;border-top:1px dashed #f0e8e0">
                            ...
                            @endphp
                            <span
                                style="background:{{ $pb }};color:{{ $pc }};font-size:0.77rem;font-weight:800;padding:5px 13px;border-radius:20px">
                        ...
                        <div class="d-flex justify-content-between align-items-center"
                            ...
                            <span
                                style="font-weight:700;color:#422018;font-size:0.88rem">{{ $order->source_label }}</span>
                        </div>
                    </div>

                    {{-- Quick payment update --}}
                    <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="mt-4 pt-3"
                        style="border-top:1px solid #f0e8e0">
                        @csrf @method('PUT')
                        ...
                        <div class="d-flex gap-2">
                            <select name="payment_status" class="form-select">
                                <option value="unpaid" {{ $order->payment_status === 'unpaid' ? 'selected' : '' }}>لم يُدفع
                                </option>
                                <option value="pending" {{ $order->payment_status === 'pending' ? 'selected' : '' }}>بانتظار
                                    تأكيد</option>
                                <option value="paid" {{ $order->payment_status === 'paid' ? 'selected' : '' }}>تم الدفع ✅
                                </option>
                            </select>
                ...
                <div class="main-table">
                    ...
                    @foreach ([['تاريخ الإنشاء', $order->created_at->format('d/m/Y H:i')], ['آخر تحديث', $order->updated_at->format('d/m/Y H:i')], ['منذ', $order->created_at->diffForHumans()]] as [$k, $v])
                        <div class="d-flex justify-content-between py-2"
                            style="border-bottom:1px solid #faf6f2;font-size:0.85rem">
