...
<div class="container-fluid px-4">
  <div class="row justify-content-center">
    <div class="col-xl-9">
      ...
      {{-- Errors --}}
      @if($errors->any())
      <div style="background:#fdf0f0;border-radius:14px;padding:14px 20px;margin-bottom:20px;margin-top:8px;color:#7b1113;font-size:0.88rem;border-right:3px solid #7b1113">
        <div style="font-weight:800;margin-bottom:6px"><i class="bi bi-exclamation-triangle-fill me-2"></i>يرجى تصحيح الأخطاء التالية:</div>
            ...
            <div class="row g-3">
              ...
              <div class="col-12">
                ...
                <textarea name="customer_address" class="form-control" rows="2"
                          placeholder="الحي، الشارع، رقم البناية...">{{ old('customer_address', $order->customer_address ?? '') }}</textarea>
              </div>
            </div>
          </div>
          ...
          <div class="mb-4 p-4 rounded-4" style="background:#fdfaf6;border:1px solid #f0e8e0">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-8">
                ...
                <select name="product_id" class="form-select" required id="productSelect" onchange="onProductChange(this)">
                  <option value="">── اختر من القائمة ──</option>
                  @foreach($products->groupBy('category') as $cat => $catProducts)
                  <optgroup label="{{ ['embroidery'=>'🪡 التطريز','handicraft'=>'✂️ أشغال يدوية','wool'=>'🧶 أعمال الصوف'][$cat] ?? $cat }}">
              ...
              <div class="col-md-8">
                ...
                <div style="background:#fff;border:1.5px solid #e8ddd4;border-radius:12px;padding:11px 15px;font-weight:700;color:#422018">
                  {{ $order->product_name }}
                  <span style="color:#aaa;font-weight:500"> — {{ number_format($order->product_price) }} ₪</span>
                </div>
              </div>
              @endif
              ...
              <div class="col-md-4">
                ...
                <input type="number" name="quantity" id="qtyInput" class="form-control"
                       ...
                       min="1" max="99" required oninput="calcTotal()">
              </div>

              {{-- Total preview (create only) --}}
              @if(!isset($order))
              <div class="col-12">
                <div id="totalBox" style="display:none;background:rgba(123,17,19,0.05);border-radius:13px;padding:14px 20px;border:1.5px solid rgba(123,17,19,0.1);display:flex;align-items:center;justify-content:space-between">
                  <span style="font-size:0.88rem;color:#7b1113;font-weight:700">الإجمالي التقديري</span>
                  <span id="totalAmt" style="font-size:1.4rem;font-weight:900;color:#7b1113">0 ₪</span>
                </div>
              </div>
              @endif
              ...
              <div class="col-md-6">
                ...
                <select name="source" class="form-select">
                  <option value="website"  {{ old('source', $order->source ?? 'website') === 'website'  ? 'selected' : '' }}>🌐 الموقع الإلكتروني</option>
                  <option value="whatsapp" {{ old('source', $order->source ?? '') === 'whatsapp' ? 'selected' : '' }}>💬 واتساب</option>
                </select>
              </div>
              ...
              <div class="col-md-6">
                ...
                <select name="payment_method" class="form-select">
                  ...
                  <option value="{{ $v }}" {{ old('payment_method', $order->payment_method ?? '') === $v ? 'selected' : '' }}>{{ $l }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="row g-3">
              <div class="col-md-4">
                ...
                <select name="status" class="form-select">
                  ...
                  <option value="{{ $v }}" {{ old('status', $order->status) === $v ? 'selected' : '' }}>{{ $l }}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-4">
                ...
                <select name="payment_status" class="form-select">
                  <option value="unpaid"  {{ old('payment_status', $order->payment_status) === 'unpaid'  ? 'selected' : '' }}>لم يُدفع</option>
                  <option value="pending" {{ old('payment_status', $order->payment_status) === 'pending' ? 'selected' : '' }}>بانتظار تأكيد</option>
                  <option value="paid"    {{ old('payment_status', $order->payment_status) === 'paid'    ? 'selected' : '' }}>تم الدفع ✅</option>
                </select>
              </div>
            </div>
          </div>
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <textarea name="notes" class="form-control" rows="3"
                          placeholder="طلبات أو تفاصيل خاصة من العميل...">{{ old('notes', $order->notes ?? '') }}</textarea>
              </div>
              <div class="col-md-6">
                ...
                <textarea name="admin_notes" class="form-control" rows="3"
                          placeholder="ملاحظاتك الخاصة على هذا الطلب...">{{ old('admin_notes', $order->admin_notes ?? '') }}</textarea>
              </div>
            </div>
          </div>
