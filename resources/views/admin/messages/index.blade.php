...
<div class="container-fluid px-4">
  <div class="d-flex justify-content-between align-items-center mb-4 mt-2">
    <div>
      ...
      <p class="text-muted small mb-0">{{ $messages->total() }} رسالة • {{ $messages->where('is_read', false)->count() }} غير مقروءة</p>
    </div>
  </div>
      ...
      <div class="msg-card {{ !$msg->is_read ? 'unread' : '' }}">
        <div class="d-flex justify-content-between align-items-start mb-3">
          <div class="d-flex align-items-center gap-2">
            <div style="width:40px;height:40px;border-radius:11px;background:rgba(123,17,19,0.1);display:flex;align-items:center;justify-content:center;color:#7b1113;font-size:1.1rem">
          ...
          <div class="d-flex align-items-center gap-2">
            @if(!$msg->is_read)
              <span style="background:#7b1113;color:#fff;font-size:0.65rem;padding:2px 8px;border-radius:20px;font-weight:700">جديد</span>
            @endif
            <small class="text-muted" style="font-size:0.72rem">{{ $msg->created_at->diffForHumans() }}</small>
          </div>
        </div>
