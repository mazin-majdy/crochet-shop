                    ...
                    <form
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="name" class="form-control"
                                    value="{{ old('name', $product->name ?? '') }}" placeholder="مثال: شال صوف مطرز"
                                    required>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="price" class="form-control" step="0.5" min="0"
                                    value="{{ old('price', $product->price ?? '') }}" placeholder="120.00" required>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="target" class="form-select">
                                    ...
                                    @foreach (['kids' => '👶 أطفال', 'girls' => '👗 بنات', 'women' => '👩 نساء', 'men' => '👔 رجال', 'general' => '🌟 عام'] as $val => $lbl)
                                        <option value="{{ $val }}"
                                            {{ old('target', $product->target ?? '') === $val ? 'selected' : '' }}>
                                            {{ $lbl }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="is_active" class="form-select">
                                    <option value="1"
                                        {{ old('is_active', $product->is_active ?? 1) == 1 ? 'selected' : '' }}>✅ نشط ومرئي
                                    </option>
                                    <option value="0"
                                        {{ old('is_active', $product->is_active ?? 1) == 0 ? 'selected' : '' }}>🔒 مخفي
                                    </option>
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="description" class="form-control" rows="4"
                                    placeholder="اكتب وصفاً تفصيلياً للمنتج — المادة، الحجم، الاستخدام..." required>{{ old('description', $product->description ?? '') }}</textarea>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="d-flex align-items-center gap-3 mt-4">
                                    <label class="d-flex align-items-center gap-2 cursor-pointer"
                                        style="font-size:0.9rem;font-weight:700;color:#422018">
                            ...
                            <div class="col-12">
                                ...
                                <div id="dropZone"
                                    style="border:2px dashed #e8ddd4;border-radius:16px;padding:32px;text-align:center;cursor:pointer;transition:0.2s;background:var(--cream)"
                                    onclick="document.getElementById('imageInput').click()">
                                    <div id="imgPreview" style="display:none;margin-bottom:12px">
                                        <img id="previewImg" style="max-height:140px;border-radius:12px;max-width:100%"
                                            alt="">
                                    </div>
                                    @if (isset($product) && $product->image)
                                        <img src="{{ asset('storage/' . $product->image) }}"
                                            style="max-height:120px;border-radius:12px;margin-bottom:12px" alt="">
                                        <br>
                                    @else
                        ...
                        <div class="d-flex gap-3 justify-content-end mt-5 pt-3 border-top">
                            <a href="{{ route('admin.products.index') }}"
                                class="btn-action py-2 px-4 text-decoration-none">إلغاء</a>
