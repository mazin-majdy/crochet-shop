        ...
        <tbody>
          ...
          <tr>
            <td>
              <div class="d-flex align-items-center gap-3">
                <div style="width:52px;height:52px;border-radius:12px;background:var(--cream);display:flex;align-items:center;justify-content:center;font-size:1.7rem;overflow:hidden;flex-shrink:0;border:1.5px solid #e8ddd4">
                  @if($product->image)
            ...
            <td><span class="badge-cat badge-{{ $product->category }}">{{ $product->category_label }}</span></td>
            ...
            <td>
              <div class="d-flex gap-2">
                <a href="{{ route('admin.products.edit', $product) }}" class="btn-action py-1 px-3 text-decoration-none">
                  <i class="bi bi-pencil"></i> تعديل
                </a>
                <form action="{{ route('admin.products.destroy', $product) }}" method="POST" onsubmit="return confirm('حذف هذا المنتج نهائياً؟')">
                  @csrf @method('DELETE')
          ...
          <tr>
            <td colspan="6" class="text-center py-5 text-muted">
              ...
              لا توجد منتجات — <a href="{{ route('admin.products.create') }}" class="text-maroon fw-bold">أضف أول منتج</a>
            </td>
          </tr>
          @endforelse
        </tbody>
