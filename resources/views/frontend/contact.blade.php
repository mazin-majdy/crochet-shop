...
<div style="background:linear-gradient(135deg,#fdf5ef,#f5e8dc);padding:52px 5% 44px;border-bottom:1px solid #eee">
  <div class="eyebrow" style="color:var(--maroon);font-size:0.78rem;font-weight:800;letter-spacing:2px;text-transform:uppercase;display:block;margin-bottom:10px">التواصل</div>
...
<section class="contact-section">
  <div class="contact-grid">
    ...
    <div class="contact-form-card">
      ...
      @if(session('success'))
      <div style="background:#edf7ef;border-radius:14px;padding:14px 18px;margin-bottom:20px;color:#1a8a4a;font-weight:700;font-size:0.9rem">
        <i class="bi bi-check-circle-fill me-2"></i> {{ session('success') }}
      </div>
      @endif

      @if($errors->any())
      <div style="background:#fdf0f0;border-radius:14px;padding:14px 18px;margin-bottom:20px;color:#7b1113;font-size:0.88rem">
      ...
      <form action="{{ route('contact.send') }}" method="POST">
        ...
        <div class="mb-4">
          ...
          <input type="text" name="name" value="{{ old('name') }}"
                 class="form-control" placeholder="ما اسمك؟" required>
        </div>
        ...
        <div class="row g-3 mb-4">
          <div class="col-6">
            ...
            <input type="tel" name="phone" value="{{ old('phone') }}"
                   class="form-control" placeholder="05xxxxxxxx" required>
          </div>
          <div class="col-6">
        ...
        <div class="mb-4">
          ...
          <textarea name="message" class="form-control" rows="5"
                    placeholder="اكتبي رسالتك هنا... أخبرينا بما تحتاجين ✨" required>{{ old('message') }}</textarea>
        </div>
    ...
    <div class="contact-info-card">
      ...
      <div style="background:var(--cream-3);border-radius:18px;padding:22px;margin-top:8px">
        ...
        @foreach(['بايبال','بنك فلسطين','أبو ظبي الأول','تحويل واتساب'] as $method)
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;font-size:0.88rem;color:#6b5040;font-weight:600">
          <i class="bi bi-check-circle-fill" style="color:#7b1113"></i> {{ $method }}
        </div>
        @endforeach
