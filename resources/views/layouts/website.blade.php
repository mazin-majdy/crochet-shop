...
<head>
    ...
    <meta name="description" content="{{ setting('site.name') }} – {{ setting('site.description') }}">
    <title>@yield('title', setting('site.name')) – أشغال يدوية وتطريز</title>
...
<body>
    ...
    <header class="site-navbar">
        ...
        <nav class="d-none d-md-flex align-items-center gap-1">
            <a href="{{ route('home') }}"
                class="{{ request()->routeIs('home') ? 'fw-800 text-maroon' : '' }}">الرئيسية</a>
            <a href="{{ route('products') }}"
                class="{{ request()->routeIs('products*') ? 'fw-800 text-maroon' : '' }}">المنتجات</a>
    ...
    <footer class="site-footer">
        <div class="footer-grid">
            <div class="footer-brand">
                <img src="{{ asset('images/logo.svg') }}" alt="{{ setting('site.name') }}">
                <h3>{{ setting('site.name') }}</h3>
                <p>متجر متخصص في الأشغال اليدوية الفاخرة، التطريز الأصيل، وأعمال الصوف المتنوعة. كل قطعة تُصنع بحب
                    وعناية.</p>
            </div>
            <div class="footer-col">
                <h4>الأقسام</h4>
                <a href="{{ route('products', ['category' => 'embroidery']) }}">التطريز</a>
                <a href="{{ route('products', ['category' => 'handicraft']) }}">الأشغال اليدوية</a>
                <a href="{{ route('products', ['category' => 'wool']) }}">أعمال الصوف</a>
                <a href="{{ route('products', ['category' => 'kids']) }}">للأطفال</a>
            </div>
            <div class="footer-col">
                <h4>روابط سريعة</h4>
            ...
            <div class="footer-col">
                ...
                <a href="{{ route('contact') }}"><i class="bi bi-envelope"></i> راسلنا</a>
            </div>
        </div>
        <div class="footer-bottom">
            <p>© {{ date('Y') }} {{ setting('site.name') }} · جميع الحقوق محفوظة -
                {{ setting('contact.address') }}</p>
            <div class="social-links">
                @if (setting('social.instagram'))
                    <a href="{{ setting('social.instagram') }}"><i class="bi bi-instagram"></i></a>
                @endif
                @if (setting('social.facebook'))
                    <a href="setting('social.facebook')"><i class="bi bi-facebook"></i></a>
                @endif
                <a href="https://wa.me/{{ config('app.whatsapp_number', setting('contact.whatsapp')) }}"
